...
% BCD 迭代参数
max_bcd_iter = 10;        % 最大迭代次数（目标：2~3次内收敛）
tol_bcd = 1e-3;            % 收敛容差（适当放宽以匹配实时/少迭代需求）

% ===== 阻尼/松弛更新参数（用于减少振荡，促进2~3次内收敛）=====
alpha_q = 0.45;   % 轨迹更新阻尼系数 (0~1)
alpha_p = 0.45;   % 功率更新阻尼系数 (0~1)
alpha_w = 0.8;   % 波束更新阻尼系数 (0~1)
% 建议：第1次迭代可取全步长，加快进入稳定区域
...
% =======================================================
    ...
    for iter = 1:max_bcd_iter
        ...
        a_q = alpha_q;  % 第1次全步长，其余阻尼
        q_w_opt = (1-a_q)*q_w_old + a_q*q_w_new;
        % --- 2.1 功率优化子问题 ---
        % 输入: 当前的轨迹 q_w_opt (此时 w_wb 不影响功率闭式解)
        P_J_new = solve_power_opt(q_w_opt, P);
        a_p = alpha_p;
        P_J_opt = (1-a_p)*P_J_old + a_p*P_J_new;
        ...
        try
            ...
            a_w = alpha_w;
            w_tmp = (1-a_w)*w_wb_old + a_w*w_wb_new;
            w_wb_opt = w_tmp / max(norm(w_tmp), 1e-12);
        catch ME
            % 保持上一次的波束
            ...
            w_wb_opt = w_wb_opt / max(norm(w_wb_opt), 1e-12); 
        end
