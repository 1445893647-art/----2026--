function [q_w_opt, v_w_opt, P_J_opt, w_wb_opt, info, hist] = solve_joint_sca_opt_with_hist(x_pred, P_pred, q_w_prev, P_J_init, w_wb_init, P)
...
W0 = w0*w0';
% ===== 迭代历史记录（用于画 SCA 收敛曲线：P_out vs SCA迭代次数）=====
hist = struct();
hist.maxIter = maxIter;
hist.q  = NaN(3, maxIter);
hist.PJ = NaN(1, maxIter);
hist.w  = NaN(Nt, maxIter);
hist.cvx_status = strings(1, maxIter);
hist.slack_ekf  = NaN(1, maxIter);
hist.trust_q = NaN(1, maxIter);
hist.trust_P = NaN(1, maxIter);
hist.trust_W = NaN(1, maxIter);
hist.init = struct('q0', q0, 'PJ0', PJ0, 'w0', w0);
...
for it = 1:maxIter
    ...
    % ---- 记录本次 SCA 迭代得到的解（用于外部绘制收敛曲线）----
    hist.q(:,it) = q0;
    hist.PJ(it)  = PJ0;
    hist.w(:,it) = w0;
    hist.cvx_status(it) = string(cvx_status);
    hist.slack_ekf(it)  = info.slack_ekf;
    hist.trust_q(it) = trust_q;
    hist.trust_P(it) = trust_P;
    hist.trust_W(it) = trust_W;
...
w_wb_opt = w0;
% ===== 输出历史记录 =====
hist.iters = info.iters;
hist.final = struct('q', q_w_opt, 'PJ', P_J_opt, 'w', w_wb_opt);
