% main_ekf_sim.m
clc; clear; close all;
config_param_sca; % 加载参数（含 SCA 参数）
...
% ===== SCA 迭代收敛记录：每个时隙、每次 SCA 迭代的 P_out =====
max_sca_iter = 10;
if isfield(P,'sca') && isfield(P.sca,'max_iter')
    max_sca_iter = P.sca.max_iter;
end
pout_sca_iter_hist = NaN(P.N_slots, max_sca_iter);
...
for n = 2:P.N_slots
    x_b_true = x_true(:,n);  % Bob 真实状态（用于评估 SCA 每次迭代的 P_out）
...
% =======================================================
    ...
    % =======================================================
    % =======================================================
    % Step 2: 联合 SCA 直接求解 (Joint SCA)
    % =======================================================
    % 目标：基于预测状态 x_pred，直接求解当前时隙最优的 {P_J, w_wb, q_w}
    % 注意：该求解器内部已做“硬检测约束 + 回溯 + 投影”，避免后续时隙越界

    try
        [q_w_opt, v_w_opt, P_J_opt, w_wb_opt, sca_info, sca_hist] = solve_joint_sca_opt_with_hist( ...
            x_pred, P_pred, q_w_prev_slot, P_J_curr, w_wb_curr, P);
    catch ME
        warning('Slot %d: SCA solver error: %s', n, ME.message);
        q_w_opt = q_w_curr;
        v_w_opt = v_w_curr;
        P_J_opt = P_J_curr;
        w_wb_opt = w_wb_curr;
        sca_info = struct('cvx_status','error','det_ub',NaN,'slack_ekf',NaN,'backtrack_trials',NaN);
        sca_hist = struct();
    end


    % ===== 记录 SCA 内迭代的 P_out（用于收敛曲线）=====
    if exist('sca_hist','var') && isstruct(sca_hist) && isfield(sca_hist,'q')
        it_used = min(sca_info.iters, max_sca_iter);
        for it2 = 1:it_used
            q_it  = sca_hist.q(:,it2);
            PJ_it = sca_hist.PJ(it2);
            w_it  = sca_hist.w(:,it2); w_it = w_it / max(norm(w_it),1e-12);

            % 检测约束“真实值”投影（与主流程一致，兜底数值误差）
            c_det = (P.P_a * P.beta_0) / (P.rho * P.Gamma_req + 1e-12);
            d_aw  = norm(P.q_a(:) - q_it) + 1e-12;
            PJ_ub_exact = min(P.P_max, c_det * d_aw^(-P.alpha_aw));
            if PJ_it > PJ_ub_exact
                PJ_it = 0.999 * PJ_ub_exact;
            end

            [p_it, ~] = calc_metrics(q_it, x_b_true(1:3), PJ_it, w_it, w_ab_dummy, P);
            pout_sca_iter_hist(n, it2) = p_it;
        end
        if it_used >= 1
            pout_sca_iter_hist(n, it_used+1:end) = pout_sca_iter_hist(n, it_used);
        end
    end
    ...
    % ===== C3 检测约束“真实值”投影（兜底防数值误差）=====
    c_det = (P.P_a * P.beta_0) / (P.rho * P.Gamma_req + 1e-12);
    d_aw  = norm(P.q_a(:) - q_w_curr) + 1e-12;  % 3D 距离
    PJ_ub_exact = min(P.P_max, c_det * d_aw^(-P.alpha_aw));
    if P_J_curr > PJ_ub_exact
        P_J_curr = 0.999 * PJ_ub_exact;
    end
    ...
    % 基于 Bob 真实位置计算无噪声测量值
    % x_b_true 已在本时隙开头定义：x_b_true = x_true(:,n);
    ...
    % 数值保护：噪声方差下界（避免后续 S 近奇异）
    var_floor = P.sca.var_floor;
    var_1_true = max(var_1_true, var_floor);
    var_2_true = max(var_2_true, var_floor);
    var_3_true = max(var_3_true, var_floor);
    ...
    % 数值保护：噪声方差下界（避免 S 近奇异）
    var_floor = P.sca.var_floor;
    var_1_p = max(var_1_p, var_floor);
    var_2_p = max(var_2_p, var_floor);
    var_3_p = max(var_3_p, var_floor);
...
% ===== Figure: SCA 收敛曲线（平均 P_out vs 迭代次数）=====
avg_pout_iter = nanmean(pout_sca_iter_hist(2:end,:), 1);
figure('Name','SCA Convergence: Avg Pout vs Iter','NumberTitle','off');
plot(1:max_sca_iter, avg_pout_iter, 'k-', 'LineWidth', 2);
ylim([0, 1.1]);
grid on; xlabel('SCA 迭代次数'); ylabel('平均中断概率');
title('SCA 收敛曲线：平均 P_{out} vs 迭代次数');
...
% ===== Baselines (no EKF, no measurement sampling) =====
%采用真实值做优化
oracle = run_noekf_opt(P, x_true, F_mat, Qx, "oracle");
%无EKF，采用上一时刻的值
lag1   = run_noekf_opt(P, x_true, F_mat, Qx, "lag1");

figure('Name','Pout Compare','NumberTitle','off'); hold on; grid on;
plot(1:P.N_slots, history_P_out,  'r-',  'LineWidth', 2);
plot(1:P.N_slots, oracle.P_out,   'b--', 'LineWidth', 2);
plot(1:P.N_slots, lag1.P_out,     'k-.', 'LineWidth', 2);
ylim([0,1.1]);
xlabel('时隙 n'); ylabel('P_{out}');
legend('EKF预测优化','真实位置优化(Oracle)','上一时刻位置优化(Lag-1)','Location','best');
title('隐蔽中断概率 P_{out} 对比');
